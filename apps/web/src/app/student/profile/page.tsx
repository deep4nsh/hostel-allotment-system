...
export default function StudentProfilePage() {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            ...
            try {
                ...
                const wlRes = await fetch('http://localhost:3000/waitlist/me', {
                    headers: { 'Authorization': `Bearer ${token}` }
                })
                if (wlRes.ok) {
    ...
    const handleDownloadAllotmentLetter = async () => {
        const token = localStorage.getItem('token')
        const res = await fetch('http://localhost:3000/letters/allotment', {
            headers: { 'Authorization': `Bearer ${token}` }
        })
        if (res.ok) {
            const blob = await res.blob()
    ...
    const handleGenerateId = async () => {
        const token = localStorage.getItem('token')
        try {
            const res = await fetch('http://localhost:3000/students/me/generate-id', {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${token}` }
            })
            if (res.ok) {
