...
export class RequestsService {
  ...
  async createChangeRequest(
    studentId: string,
    data: { reason: string; preferredHostelId?: string },
  ) {
    // Check student year
    ...
    if (student.year === 1) {
      throw new Error(
        'First year students are not allowed to request hostel changes.',
      );
    }
  ...
  async createSurrenderRequest(
    studentId: string,
    data: { reason: string; clearanceUrl?: string },
  ) {
    const allotment = await this.prisma.allotment.findUnique({
      where: { studentId },
    });
  ...
  async updateSurrenderRequestStatus(
    id: string,
    status: string,
    comment?: string,
  ) {
