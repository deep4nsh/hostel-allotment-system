...
describe('Hostel Rules Verification', () => {
  ...
  beforeAll(async () => {
    ...
    // Clean DB - Order matters for FK
    await prisma.roomSwapInvite.deleteMany();
    ...
    // Student must be deleted after payments/allotments but before User
    // But Hostel has Wardens (User).
    // And Hostel logic might references user? No.
    // However, Hostel has `wardens User[]`.
    // It's a many-to-many or implicit? schema says `wardens User[] @relation("HostelWarden")`
    // User says `hostels Hostel[] @relation("HostelWarden")`
    // Implicit many-to-many table `_HostelWarden`.
    // Deleting Hostel or User should handle cascading in implicit table?
    // Prisma usually handles implicit M-N cascade but let's see.
  ...
  it('should prioritize PH students in allotment', async () => {
    ...
    const u1 = await prisma.user.create({
      data: { email: `u1_${Date.now()}@ex.com`, password: 'pw' },
    });
    const u2 = await prisma.user.create({
      data: { email: `u2_${Date.now()}@ex.com`, password: 'pw' },
    });
    ...
    // Run Allotment
    const result = await allotmentService.runAllotment(1);
