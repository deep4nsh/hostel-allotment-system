...
export class AllotmentService {
  ...
  async runAllotment(targetYear: number) {
    try {
      ...
      const isRoomTypeAllowed = (
        room: any,
        student: any,
        hostelIsAC: boolean,
      ) => {
        const level =
          getProgramGroup(student.program) === 'POSTGRAD'
            ? 'MASTER'
            : 'BACHELOR'; // Simplified util wrapper
      ...
      for (const student of eligibleStudents) {
        ...
        const effectiveCategory =
          sCategory === 'NRI' || isInternational ? 'NRI' : sCategory;
