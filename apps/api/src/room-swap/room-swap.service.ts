...
export class RoomSwapService {
    ...
    async createListing(userId: string) {
        ...
        if (!student || !student.allotment || !student.allotment.isPossessed) {
            throw new BadRequestException('You must have a possessed room allotment to list for swap.');
        }
    ...
    async sendInvite(userId: string, targetStudentId: string) {
        ...
        if (existing) {
            throw new BadRequestException('A pending invite already exists between you.');
        }
    ...
    async respondToInvite(userId: string, inviteId: string, status: 'ACCEPTED' | 'REJECTED') {
        ...
        if (invite.receiver.userId !== userId) throw new BadRequestException('Not authorized to respond to this invite');
        ...
        if (!invite.sender.allotment || !invite.receiver.allotment) {
            throw new BadRequestException('One or both students have lost their allotment. Swap failed.');
        }
